...
export function IconCloud({
  ...
  useEffect(() => {
    ...
    const newIconCanvases = items.map((item, index) => {
      ...
      // --- التعديل هنا: تصغير حجم الأيقونة لخلق مسافات ---
      offscreen.width = 40  
      offscreen.height = 40 
      // --------------------------------------------------
      ...
      if (offCtx) {
        if (images) {
          ...
          img.onload = () => {
            ...
            offCtx.beginPath()
            offCtx.arc(20, 20, 20, 0, Math.PI * 2) // نصف القطر 20
            offCtx.closePath()
            offCtx.clip()
            offCtx.drawImage(img, 0, 0, 40, 40) // رسم بحجم 40
  ...
  // ... (باقي كود useEffect الخاص بالحسابات ومتابعة الماوس كما هو بدون تغيير) ...
  useEffect(() => {
    const items = icons || images || []
    const newIcons = []
      ...
      iconPositions.forEach((icon, index) => {
        ...
        if (icons || images) {
          if (iconCanvasesRef.current[index] && imagesLoadedRef.current[index]) {
            // --- التعديل هنا: الرسم بحجم 40 للحفاظ على المسافات ---
